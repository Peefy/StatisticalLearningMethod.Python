...
class Chapter1:
    ...
    def note(self):
        ...
        print('1.9 标注问题')
        print('标注(tagging)也是一个监督学习问题,可以认为标注问题是分类问题的一个推广',
            '标注问题又是更复杂的结构预测问题的简单形式.标注问题的输入是一个观测序列,输出是一个标记序列或状态序列')
        print('标注问题的目标在于学习一个模型,使它能够对观测序列给出标记序列作为预测.',
            '注意,可能标记的个数是有限的,但其组合所成的标记序列的个数时依序列长度呈指数级增长的')
        print('标注问题分为学习和标注两个过程:')
        print('  首先给定一个训练数据集.这里,xi是输入观测序列,yi是相应的输出标记序列,n是序列的长度,对不同样本可以有不同的值.',
            '学习系统基于训练数据集构建一个模型,表示为条件概率分布P(Y(1),Y(2),...,Y(n)|X(1),X(2),...,X(n))')
        print('  这里,每一个X(i)(i=1,2,...,n)取值为所有可能的观测,每一个Y(i)(i=1,2,...,n)取值为所有可能的标记,',
            '一般n<<N.标注系统按照学习得到的条件概率分布模型,对新的输入观测序列找到相应的输出标记序列.具体地,对一个观测序列xN+1',
            '找到使条件概率P((yN+1(1),yN+1(2),...,yN+1(n))^T|(xN+1(1),xN+1(2),...,xN+1(n))^T))',
            '最大的标记序列yN+1=(yN+1(1),yN+1(2),...,yN+1(n))^T')
        print('评价标注模型的指标与评价分类模型的指标一样,常用的有标注准确率、精确率和召回率.其定义与分类模型相同')
        print('标注常用的统计学习方法有:隐马尔可夫模型、条件随机场')
        print('标注问题在信息抽取、自然语言处理等领域被广泛应用,例如自然语言处理中的词性标注就是一个典型的标注问题')
        print('1.10 回归问题')
        print('回归(regression)是监督学习的另一个重要问题.回归用于预测输入变量(自变量)和输出变量(因变量)之间的关系,',
            '特别是当输入变量的值发生变化时,输出变量的值随之发生的变化.回归模型正是表示从输入变量到输出变量之间的映射的函数',
            '回归问题的学习等价于函数拟合:选择一条函数曲线使其很好地拟合已知数据且很好地预测未知数据')
        print('回归问题分为学习和预测两个过程:')
        print('  首先给定一个训练数据集:这里xi∈Rn是输入,y∈R是对应的输出,i=1,2,...,N.学习系统基于训练数据构建一个模型,',
            '即函数Y=f(X);对新的输入xN+1,预测系统根据学习的模型Y=f(X)确定相应的输出yN+1')
        print('回归问题按照输入变量的个数,分为一元回归和多元回归；按照输入变量和输出变量之间关系的类型即模型的类型,',
            '分为线性回归和非线性回归')
        print('回归学习最常用的损失函数是平方损失函数,在此情况下,回归问题可以由最小二乘法求解')
        print('回归问题举例：可以用于商务领域,作为市场趋势预测、产品质量管理、客户满意度调查、投资风险分析的工具')
        print('具体举例：将影响股价的信息看作自变量(输入的特征),而将股价视为因变量(输出的值).将过去的数据作为训练数据,',
            '就可以学习一个回归模型,并对未来股价进行预测.可以看出这一个困难的预测问题',
            '因为影响股价的因素非常多,未必能判断到哪些信息(输入的特征)有用并能得到这些信息')
        print('本章概要')
        print('1.统计学习是关于计算机基于数据构建概率统计模型并运用模型对数据进行分析与预测',
            '的一门学科。统计学习包括监督学习、非监督学习、半监督学习和强化学习')
        print('2.统计学习方法三要素----模型、策略、算法')
        print('3.监督学习概括：从给定有限的训练数据出发，假设数据是独立同分布的，而且假设模型数据某个假设空间，',
            '应用评价准则，从假设空间中选取一个最优的模型，使它对已给训练数据及未知测试数据在给定评价标准意义下有最准确的预测')
        print('4.统计学习中，进行模型选择或者说提高学习的泛化能力是一个重要问题.如果只考虑减少训练误差,就可能产生过拟合现象。',
            '模型选择的方法有正则化与交叉验证。学习方法泛化能力的分析使统计学习理论研究的重要课题')
        print('5.分类问题、标注问题和回归问题都是监督学习的重要问题。统计学习方法包括感知机、k近邻法、朴素贝叶斯法、',
            '决策树、Logistic回归、最大熵模型、支持向量机、提升方法、EM算法、隐马尔可夫模型和条件随机场。',
            '这些是主要的分类、标注以及回归方法。它们又可以归类为生成方法与判别方法')
        print('习题1.1 说明伯努利模型的极大似然估计以及贝叶斯估计中的统计学习方法三要素.',
            '伯努利模型是定义在取值为0与1的随机变量上的概率分布.假设观测到伯努利模型n次独立的数据生成结果',
            '其中k次的结果为1,这时可以用极大似然估计或贝叶斯估计结果为1的概率')
        print('习题1.2 通过经验风险最小化推导极大似然估计.证明模型是条件概率分布,当损失函数是对数损失函数时,',
            '经验风险最小化等价于极大似然估计')
